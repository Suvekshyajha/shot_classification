...
class PadelPipeline:
    ...
    def __init__(self, model_path="yolov8n.pt", conf=0.35, annotate=True, output_dir="outputs"):
        ...
        with tqdm(total=total_frames, desc="Processing", unit="frame") as bar:
            while True:
                ...
                if event:
                    ...
                    logger.debug(f"Shot {event.shot_id} | {event.shot_type.value} | {timestamp:.2f}s")
    ...
    def _save_csv(self, events, path):
        if not events:
            logger.warning("No shots — CSV not saved")
            return
